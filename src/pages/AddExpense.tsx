...
import { listMembers } from '../lib/github'
    ...
    <div>
      ...
      <div className="space-y-5">
        ...
        <div>
          <label className="block text-sm font-medium text-zinc-700 mb-1.5">
            Split between
            <span className="ml-1 text-zinc-400 font-normal text-xs">(equal split)</span>
          </label>
          {membersLoading ? (
            <Spinner className="py-4" />
          ) : (
            <div className="space-y-2">
              {(members ?? []).map(m => (
                <button
                  key={m.login}
                  onClick={() => toggleParticipant(m.login)}
        ...
        {isValid && (
          <div className="bg-zinc-50 border border-zinc-200 rounded-xl p-4 text-sm text-zinc-600">
            ...
            {' '}for {participants.size} people.
            Each owes <span className="font-medium text-emerald-600">{formatAmount(perPerson, currency)}</span>.
          </div>
        )}
